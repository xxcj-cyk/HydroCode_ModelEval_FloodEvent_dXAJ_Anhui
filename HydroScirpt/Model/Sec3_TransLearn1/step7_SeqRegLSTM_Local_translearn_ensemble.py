...
RESULT_ROOT = ROOT / "HydroScirpt/Result/Sec3_TransLearn1/SeqRegLSTM_Local_TransLearn_reuse"
...
def build_ensemble_job() -> EnsembleJob:
    ...
    return EnsembleJob(
        ...
        summary_csv_name=f"{MODEL_TAG}_TransLearn_reuse_Ensemble_Summary.csv",
        contrib_csv_name=(
            f"{MODEL_TAG}_TransLearn_reuse_Ensemble_Median_Contribution.csv"
        ),
    )
...
def run_median_ensemble(job: EnsembleJob, replace_basin: str | None = None) -> None:
    ...
    print("=" * 72)
    print(f"Sec3 — {MODEL_TAG} translearn median ensemble [{job.mode}]")
