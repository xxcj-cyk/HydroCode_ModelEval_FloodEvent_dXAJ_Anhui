...
TRAIN_PREFIX = "dXAJ-mz_Train"
TRANSLEARN_PREFIX = "dXAJ-mz_TransLearn"
TOP9_CSV = (
    ROOT
...
SUBSETS = {
    "398": {
        ...
        "train_dir": ROOT / "HydroScirpt/Result/Sec3_TransLearn2/dXAJ-mz_Train_Anhui398",
        "output_dir": r"HydroScirpt/Result/Sec3_TransLearn2/dXAJ-mz_TransLearn_reuse_Anhui398",
        "max_workers": 10,
    },
    "535": {
        "input_path": f"{DATASET_ROOT}/Anhui18_535",
        "train_dir": ROOT / "HydroScirpt/Result/Sec3_TransLearn2/dXAJ-mz_Train_Anhui535",
        "output_dir": r"HydroScirpt/Result/Sec3_TransLearn2/dXAJ-mz_TransLearn_reuse_Anhui535",
        "max_workers": 5,
    },
}
...
def resolve_transfer_artifacts(
    subset: str, target_basin: str, run_name: str
) -> tuple[str, str]:
    src_dir = (
        SUBSETS[subset]["train_dir"]
        / f"{TRAIN_PREFIX}_{target_basin}_without_{run_name}"
    )
...
def base_config(subset: str) -> dict:
    return {
        "data_cfgs": {
            "input_path": SUBSETS[subset]["input_path"],
            "output_dir": SUBSETS[subset]["output_dir"],
            "basin_ids": None,
            "variables": {
...
def build_sweep(subset: str, test_mode: bool) -> list[tuple]:
    all_event_ids = glob_basin_ids(SUBSETS[subset]["input_path"])
...
def build_experiment_config(
    ...
    model_path, scaler_path = resolve_transfer_artifacts(subset, target_basin, run_name)
...
def run_subset(subset: str, test_mode: bool) -> tuple[int, int]:
    ...
    print("=" * 60)
    print(f"Sec3_TransLearn2 — dXAJ-mz Regional LOO translearn (Anhui{subset})")
    print(f"Top9 CSV: {TOP9_CSV}")
    print(f"Train dir: {SUBSETS[subset]['train_dir']}")
