...
PROJECT_PREFIX = "SeqRegLSTM_Regional"
TOP9_CSV = (
    ROOT
...
SUBSETS = {
    "398": {
        ...
        "output_dir": r"HydroScirpt/Result/Sec3_TransLearn2/SeqRegLSTM_Regional_Anhui398",
        "max_workers": 9,
    },
    "535": {
        "input_path": f"{DATASET_ROOT}/Anhui18_535",
        "output_dir": r"HydroScirpt/Result/Sec3_TransLearn2/SeqRegLSTM_Regional_Anhui535",
        "max_workers": 9,
    },
}
...
def build_sweep(subset: str, test_mode: bool) -> list[tuple]:
    ...
    return sweep[:1] if test_mode else sweep
...
def build_experiment_config(
    ...
    cfg["training_cfgs"]["experiment_name"] = f"{PROJECT_PREFIX}_{run_name}"
...
def run_single_scheme(args: tuple) -> bool:
    subset, hidden_size, dropout, warmup_length, batch_size, lr, seed = args
    run_name = build_run_name(hidden_size, dropout, warmup_length, batch_size, lr, seed)
    label = f"Anhui{subset} / {run_name}"
    try:
    ...
    with ProcessPoolExecutor(max_workers=worker_num, mp_context=ctx) as executor:
        ...
        for future in as_completed(futures):
            try:
                ...
                failed += 1
                args = futures[future]
                print(f"Error: Anhui{args[0]} -> {exc}")
...
def run_subset(subset: str, test_mode: bool) -> tuple[int, int]:
    ...
    print("=" * 60)
    print(f"Sec3_TransLearn2 — SeqRegLSTM unified regional train Top9 (Anhui{subset})")
...
def main() -> None:
    ...
    parser.add_argument(
        "--test",
        action="store_true",
        help="Run only the Top1 hyperparam config",
    )
