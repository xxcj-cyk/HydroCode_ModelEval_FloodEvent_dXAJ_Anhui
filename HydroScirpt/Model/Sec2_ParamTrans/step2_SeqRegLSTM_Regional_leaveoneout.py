...
PROJECT_PREFIX = "SeqRegLSTM_Train"
TOP9_CSV = (
    ROOT
...
BASE_CONFIG = {
    "data_cfgs": {
        ...
        "output_dir": r"HydroScirpt/Result/Sec2_ParamTrans/SeqRegLSTM_Train",
        "basin_ids": None,
        "variables": {
    ...
    "training_cfgs": {
        ...
        "experiment_name": (
            f"{PROJECT_PREFIX}_Anhui_50406910_without_h128_dr00_wu144_b0064_lr005_seed1111"
        ),
        "loss_function": "RMSE",
        "optimizer_name": "Adam",
        "learning_rate": 0.05,
    ...
    with ProcessPoolExecutor(max_workers=worker_num, mp_context=ctx) as executor:
        ...
        for future in as_completed(futures):
            try:
                ...
                failed += 1
                args = futures[future]
                print(f"Error: {args[0]} / {build_run_name(*args[2:])} -> {exc}")
...
def main() -> None:
    ...
    train_events = len(sweep[0][1]) if sweep else 0

    print("=" * 60)
    print("Sec2 — SeqRegLSTM leave-one-basin-out regional train")
