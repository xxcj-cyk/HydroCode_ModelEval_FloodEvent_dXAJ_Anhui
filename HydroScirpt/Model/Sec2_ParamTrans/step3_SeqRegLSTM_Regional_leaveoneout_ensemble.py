...
RESULT_ROOT = ROOT / "HydroScirpt/Result/Sec2_ParamTrans/SeqRegLSTM_Train"
ENSEMBLE_DIRNAME = "Ensemble"
SCHEME_PREFIX = "SeqRegLSTM_Train_"
...
class EnsembleJob:
    ...
    def ensemble_dir_for(self, target_basin: str) -> Path:
        return self.ensemble_root / f"{SCHEME_PREFIX}{target_basin}_without"


def build_ensemble_job() -> EnsembleJob:
    pattern = re.compile(
        rf"^{re.escape(SCHEME_PREFIX)}(Anhui_\d+)_without_(h\d+_.+)$"
    )
    return EnsembleJob(
        mode="train",
        result_root=RESULT_ROOT,
        scheme_pattern=pattern,
        summary_csv_name=f"{MODEL_TAG}_Train_LeaveOneOut_Ensemble_Summary.csv",
        contrib_csv_name=(
            f"{MODEL_TAG}_Train_LeaveOneOut_Ensemble_Median_Contribution.csv"
        ),
    )
...
def run_median_ensemble(job: EnsembleJob, replace_basin: str | None = None) -> None:
    ...
    print("=" * 72)
    print(f"Sec2 — {MODEL_TAG} leave-one-out median ensemble [{job.mode}]")
...
def main() -> None:
    ...
    setup_logging()
    run_median_ensemble(build_ensemble_job(), replace_basin=args.test)
