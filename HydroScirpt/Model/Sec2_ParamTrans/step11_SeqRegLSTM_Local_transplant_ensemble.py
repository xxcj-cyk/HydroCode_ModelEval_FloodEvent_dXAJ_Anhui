...
RESULT_ROOT = ROOT / "HydroScirpt/Result/Sec2_ParamTrans/SeqRegLSTM_Local_Transplant_reuse"
...
def build_ensemble_job() -> EnsembleJob:
    ...
    return EnsembleJob(
        ...
        summary_csv_name=f"{MODEL_TAG}_Transplant_reuse_Ensemble_Summary.csv",
        contrib_csv_name=(
            f"{MODEL_TAG}_Transplant_reuse_Ensemble_Median_Contribution.csv"
        ),
    )
...
def run_median_ensemble(job: EnsembleJob, replace_basin: str | None = None) -> None:
    ...
    print("=" * 72)
    print(f"Sec2 — {MODEL_TAG} transplant median ensemble [{job.mode}]")
