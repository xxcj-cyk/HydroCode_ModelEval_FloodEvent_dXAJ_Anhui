...
RESULT_ROOT = ROOT / "HydroScirpt/Result/Sec4_Discussion/Sec2_ParamTrans/SeqRegLSTM_Local_Transplant_extend"
ENSEMBLE_DIRNAME = "Ensemble"
SCHEME_PREFIX = "SeqRegLSTM_LocalTransplant_"
...
def build_ensemble_job() -> EnsembleJob:
    ...
    return EnsembleJob(
        ...
        summary_csv_name=f"{MODEL_TAG}_Transplant_extend_Ensemble_Summary.csv",
        contrib_csv_name=(
            f"{MODEL_TAG}_Transplant_extend_Ensemble_Median_Contribution.csv"
        ),
    )
...
def run_median_ensemble(job: EnsembleJob, replace_basin: str | None = None) -> None:
    ...
    print("=" * 72)
    print(f"Sec4 — {MODEL_TAG} transplant median ensemble [{job.mode}]")
