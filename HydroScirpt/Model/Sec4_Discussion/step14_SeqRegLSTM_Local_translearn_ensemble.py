...
MODEL_TAG = "SeqRegLSTM_Local"
RESULT_ROOT = ROOT / "HydroScirpt/Result/Sec4_Discussion/Sec3_TransLearn1/SeqRegLSTM_Local_TransLearn_extend"
ENSEMBLE_DIRNAME = "Ensemble"
SCHEME_PREFIX = "SeqRegLSTM_LocalTransLearn_"
...
def build_ensemble_job() -> EnsembleJob:
    ...
    return EnsembleJob(
        ...
        summary_csv_name=f"{MODEL_TAG}_TransLearn_extend_Ensemble_Summary.csv",
        contrib_csv_name=(
            f"{MODEL_TAG}_TransLearn_extend_Ensemble_Median_Contribution.csv"
        ),
    )
...
def run_median_ensemble(job: EnsembleJob, replace_basin: str | None = None) -> None:
    ...
    print("=" * 72)
    print(f"Sec4 — {MODEL_TAG} translearn median ensemble [{job.mode}]")
