...
HYPERPARAMS_CSV = (
    Path(__file__).resolve().parents[2] / "Data" / "sampled_hyperparams_400.csv"
)
MAX_WORKERS = 6
...
BASE_CONFIG = {
    "data_cfgs": {
        ...
        "output_dir": r"HydroScirpt/Result/Sec0_ModelConfig/SeqRegLSTM_Local",
        "basin_ids": None,
        "variables": {
    ...
    "training_cfgs": {
        "strategy": "LocalTrain",
        "experiment_name": "SeqRegLSTM_Local_h128_dr00_wu144_b0064_lr005_seed1111",
        "loss_function": "RMSE",
        "optimizer_name": "Adam",
        "learning_rate": 0.05,
...
def main() -> None:
    ...
    parser.add_argument(
        "--test",
        action="store_true",
        help="Run only the first hyperparam scheme (18 basins, LocalTrain)",
    )
    ...
    print("=" * 60)
    print("Sec0 — SeqRegLSTM LocalTrain hyperparam sweep")
    print(f"Hyperparams CSV: {HYPERPARAMS_CSV}")
    print(f"Output dir: {BASE_CONFIG['data_cfgs']['output_dir']}")
    print(f"Total runs: {len(sweep)} (each trains 18 basins locally)")
    print(f"Workers: {min(workers, len(sweep))}")
    print("-" * 60)
