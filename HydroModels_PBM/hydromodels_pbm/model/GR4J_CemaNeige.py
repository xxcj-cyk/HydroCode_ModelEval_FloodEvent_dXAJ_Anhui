...
from hydromodels_pbm.model.GR4J import simulate as gr4j_simulate
...
def gr4j_cemaneige(drivers, parameters, warmup_length=365):
    if drivers.shape[2] != 3:
        raise ValueError(
            'GR4J_CemaNeige expects drivers [time, basin, 3]: P, PET, T '
    ...
    pliq = cemaneige_series(p, temp, params[:, 4], params[:, 5], gthreshold)
    p_and_e = np.empty((drivers.shape[0], drivers.shape[1], 2), dtype=np.float64)
    p_and_e[:, :, 0] = pliq
    p_and_e[:, :, 1] = pet
    qsim, ets = gr4j_simulate(
        p_and_e, params[:, 0], params[:, 1], params[:, 2], params[:, 3]
    )
