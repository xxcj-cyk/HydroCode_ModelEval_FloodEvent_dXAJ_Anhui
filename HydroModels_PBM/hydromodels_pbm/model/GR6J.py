...
@jit(nopython=True)
def route_store_step(q9, q1, x2, x3, x5, x6, r, r_exp, split_coef):
    if r < 0.0:
        r = 0.0
    if r > x3:
        r = x3
    exchange = x2 * r / x3 - x2 * x5
    r_new = r + q9 * (1.0 - split_coef) + exchange
    if r_new < 0.0:
        exchange = -(r + q9 * (1.0 - split_coef))
        r_new = 0.0
    r = r_new
    ...
    r_exp = r_exp + q9 * split_coef + exchange
    qr_exp, r_exp = exponential_outflow(r_exp, x6)
    return q_nl + qr_exp, r, r_exp
...
@jit(nopython=True)
def simulate(p_and_e, x1, x2, x3, x4, x5, x6):
    ...
    for t in range(n_time):
        for j in range(n_basin):
            qsim[t, j], r[j], r_exp[j] = route_store_step(
                q9[t, j],
                q1[t, j],
                x2[j],
                x3[j],
...
def gr6j(p_and_e, parameters, warmup_length=365):
