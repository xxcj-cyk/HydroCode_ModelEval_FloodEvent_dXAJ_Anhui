...
from hydromodels_pbm.model.GR5J import simulate as gr5j_simulate
...
def gr5j_cemaneige(drivers, parameters, warmup_length=365):
    if drivers.shape[2] != 3:
        raise ValueError(
            'GR5J_CemaNeige expects drivers [time, basin, 3]: P, PET, T '
    ...
    pliq = cemaneige_series(p, temp, params[:, 5], params[:, 6], gthreshold)
    p_and_e = np.empty((drivers.shape[0], drivers.shape[1], 2), dtype=np.float64)
    p_and_e[:, :, 0] = pliq
    p_and_e[:, :, 1] = pet
    qsim, ets = gr5j_simulate(
        p_and_e,
        params[:, 0],
        params[:, 1],
        params[:, 2],
